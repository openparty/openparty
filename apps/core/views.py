...
def index(request):
    topic_list = Topic.objects.all().order_by(
...
class TopicList(ListView):
    queryset = Topic.objects.filter(accepted=True).order_by(
        "-in_event__begin_time", "-accepted", "-total_votes"
    )
...
def join_event(request):
    if not request.user.is_authenticated():
        messages.info(
            request, u'对不起，您需要先登录才能报名参加活动，如果没有帐号可以选择<a href="/member/signup">注册</a>'
        )
...
def list_post(request):
    all_post = get_list_or_404(
        Post.objects.order_by("-created_at"), status=Post.post_status.OPEN
    )
