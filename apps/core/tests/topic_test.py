...
class TopicTest(TestCase):
    def test_topic_summary(self):
        ...
        self.assertEquals(
            u"软件需求遇到的最大问题是什么？基本上都是沟通和交流的相关问题需求从哪里来：客户（市场）、用户 我们需要确定的是：谁是用户...",
            t.summary,
        )
    ...
    def test_captcha_on_submit(self):
        ...
        response = self.client.post(
            reverse("submit_new_topic"),
            {
                ...
                "content": "content",
                "in_event": event.id,
                "captcha": "should be empty if human",
            },
        )
    ...
    def test_edit_topic(self):
        ...
        test_user_topic = Topic.objects.create(
            author=member_new_user,
            in_event=event,
            name="Test",
            description="test",
            content="test",
        ...
        non_relevant_user = User.objects.create_user(
            "another_user", "another@test.com", "123"
        )
        member_non_relevant_user = Member.objects.create(
            user=non_relevant_user, nickname="Another"
        )

        test_non_user_topic = Topic.objects.create(
            author=member_non_relevant_user,
            in_event=event,
            name="Another Topic",
            description="test",
            content="test",
