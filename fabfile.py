...
def symlink_current_release():
  ...
  with settings(warn_only=True):
    run('cd %(path)s/media; ln -s %(path)s/shared/post_images post_images')
    run('cd %(path)s/media; ln -s %(path)s/shared/upload upload')
    run('cd %(path)s/media; ln -s /usr/local/lib/python2.7/dist-packages/django/contrib/admin/static/admin .')
