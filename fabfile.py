...
def clone_repo():
    ...
    run(
        "cd %(path)s; git clone https://github.com/openparty/openparty.git repository"
        % env
    )
...
def checkout_latest():
    ...
    run(
        "cp -R %(path)s/repository %(path)s/releases/%(release)s; rm -rf %(path)s/releases/%(release)s/.git*"
        % env
    )
...
def symlink_current_release():
    ...
    with settings(warn_only=True):
        run(
            "cd %(path)s; rm releases/previous; mv releases/current releases/previous;"
            % env
        )
    run("cd %(path)s; ln -s %(release)s releases/current" % env)
    """ production settings"""
    run(
        "cd %(path)s/releases/current/; cp %(path)s/conf/local_settings.py local_settings.py"
        % env
    )
    run(
        "cd %(path)s/releases/current/; cp %(path)s/conf/site-restart site-restart"
        % env
    )
    with settings(warn_only=True):
        run(
            "cd %(path)s/releases/current/media; ln -s %(path)s/shared/post_images post_images"
            % env
        )
        run(
            "cd %(path)s/releases/current/media; rm -rf upload; ln -s %(path)s/shared/upload upload"
            % env
        )
        run(
            "cd %(path)s/releases/current/media; ln -s /usr/local/lib/python2.7/dist-packages/django/contrib/admin/static/admin ."
            % env
        )
...
def migrate():
    """Run our migrations"""
    run(
        "cd %(path)s/releases/current; python manage.py syncdb --noinput --migrate"
        % env
    )
...
def restart_server():
    ...
    with activate_virtualenv():
        run(
            "cd %(path)s/releases/current; %(path)s/releases/current/site-restart" % env
        )
...
def activate_virtualenv():
    return prefix(
        "PATH=/home/%(user)s/.pythonbrew/bin:$PATH source /home/%(user)s/.pythonbrew/etc/bashrc "
        "&& pythonbrew use %(python_version)s && pythonbrew venv use %(name)s-%(settings)s"
        % env
    )
...
def create_venv():
    with prefix(
        "PATH=/home/%(user)s/.pythonbrew/bin:$PATH source /home/%(user)s/.pythonbrew/etc/bashrc && pythonbrew use %(python_version)s"
        % env
    ):
